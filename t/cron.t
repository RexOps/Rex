...
is(
  $cron[8]->{cron}->{day_of_week}, "*",
  "the first job / day_of_month of week"
);
is(
  $cron[8]->{cron}->{command}, "/home/username/script/script1.sh > /dev/null",
  "the first job / cmd"
);
...
is(
  $cron[9]->{cron}->{day_of_week}, "*",
  "the 2nd job / day_of_month of week"
);
is( $cron[9]->{cron}->{command}, "/usr/bin/script2.sh > /dev/null 2>&1",
  "the 2nd job / cmd" );
...
is(
  $cron[10]->{cron}->{day_of_week}, "0,4",
  "the 3rd job / day_of_month of week"
);
is( $cron[10]->{cron}->{command}, "cp /pfad/zu/datei /pfad/zur/kopie",
  "the 3rd job / cmd" );
...
unless ( $^O =~ m/^MSWin/ ) {
  ...
  is(
    $cron[9]->{cron}->{day_of_week}, "0,4",
    "the 3rd job / day_of_month of week"
  );
  is( $cron[9]->{cron}->{command}, "cp /pfad/zu/datei /pfad/zur/kopie",
    "the 3rd job / cmd" );
  ...
  is( $cron[0]->{command}, "/home/username/script/script1.sh > /dev/null",
    "the first job / cmd" );
  ...
  is(
    $cron[1]->{day_of_week}, "0,4",
    "the second job / day_of_month of week"
  );
  is( $cron[1]->{command}, "cp /pfad/zu/datei /pfad/zur/kopie",
    "the second job / cmd" );
