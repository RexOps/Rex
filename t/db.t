...
sub _test_select {
    _initalize_db();
    my @data = db('select' => { fields => '*', from => 'mytest', where => 'id=2' });
    is($data[0]->{mykey}, "second", "correct value for id 2");
    @data = db('select' => { fields => '*', from => 'mytest', where => 'id=5' });
...
sub _test_insert {
    ...
    my @data = db('select' => { fields => '*', from => 'mytest', where => 'id=5' });
...
sub _test_delete {
    ...
    my @data = db('select' => { fields => '*', from => 'mytest', where => 'id=1' });
...
sub _initalize_db {
    ...
    ok($dbh->do('CREATE TABLE mytest (id INT  primary key, mykey varchar(64))'), "table mytest created");
